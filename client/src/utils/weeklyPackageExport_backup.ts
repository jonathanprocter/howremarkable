...
/**
 * Draw exact weekly dashboard matching the main application
 */
const drawExactWeeklyDashboard = async (
  pdf: jsPDF,
  weekStartDate: Date,
  weekEndDate: Date,
  events: CalendarEvent[]
  ...
  pdf.setFontSize(18);
  pdf.text('Weekly Overview', pageWidth / 2, 30, { align: 'center' });
  ...
  // Draw weekly grid
  drawWeeklyGrid(pdf, config, weekStartDate, weekEndDate, events);
};

/**
 * Draw weekly grid with time slots and events
 */
const drawWeeklyGrid = (
  pdf: jsPDF,
  config: typeof WEEKLY_CONFIG,
  weekStartDate: Date,
  weekEndDate: Date,
  events: CalendarEvent[]
): void => {
  ...
  const totalSlots = 35; // 6:00 AM to 11:30 PM (17.5 hours * 2 slots per hour)
